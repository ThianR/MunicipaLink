            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; gap: 0.5rem;">
                        <button class="button button--secondary btn-sm btn-edit-muni" data-id="${m.id}" data-obj='${JSON.stringify(m)}' title="Editar Muni">
                            <i data-lucide="settings" style="width: 16px; height: 16px;"></i>
                        </button>
                        <button class="button button--primary btn-sm btn-manage-depts" data-id="${m.id}" data-name="${m.nombre}" title="Gestionar Departamentos" style="display: flex; align-items: center; gap: 4px;">
                            <i data-lucide="building" style="width: 16px; height: 16px;"></i> Deptos
                        </button>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                <td style="padding: 0.75rem 1rem;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        <div style="width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 0.8125rem;">
                            ${initial}
                        </div>
                        <div>
                            <strong style="display: block; font-size: 0.875rem;">${u.alias || 'Sin alias'}</strong>
                            <small style="color: var(--text-muted); font-size: 0.75rem;">${u.nombre_completo || 'Sin nombre'}</small>
                        </div>
                    </div>
                </td>
                <td style="padding: 0.75rem 1rem;"><small style="font-family: monospace; font-size: 0.75rem; color: var(--text-muted);">${u.id.substring(0, 8)}...</small></td>
                <td style="padding: 0.75rem 1rem;">
                ...
                <td style="padding: 0.75rem 1rem; font-size: 0.8125rem;">${u.nivel || 'Vecino Novato'} <small style="color: var(--text-muted);">(${u.puntos || 0} XP)</small></td>
                <td style="padding: 0.75rem 1rem; font-size: 0.8125rem; color: var(--text-muted);">${fecha}</td>
                <td style="padding: 0.75rem 1rem;">
                    <div style="display: flex; gap: 0.5rem;">
                        <button class="button button--secondary btn-sm btn-edit-user" data-id="${u.id}" data-obj='${JSON.stringify(u).replace(/'/g, "&#39;")}' title="Editar">
                            <i data-lucide="edit-2" style="width: 14px; height: 14px;"></i>
                        </button>
                        <button class="button btn-sm btn-reset-password" data-id="${u.id}" data-email="${u.email}" data-alias="${u.alias || u.nombre_completo}" title="Resetear Contrasena" style="background: #fbbf24; color: white;">
                            <i data-lucide="key" style="width: 14px; height: 14px;"></i>
                        </button>
                        <button class="button button--danger btn-sm btn-ban-user" data-id="${u.id}" data-activo="${u.activo !== false}" title="${u.activo === false ? 'Desbanear' : 'Banear'}">
...
async function cargarSolicitudesRol() {
    ...
    listEl.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem; color: var(--text-muted);">Cargando solicitudes...</td></tr>';
    ...
    try {
        const { data, error } = await supabaseClient
            ...
            .select(`
                *,
                perfiles:usuario_id (alias, nombre_completo, email),
...
function renderSolicitudes(data) {
    ...
    if (data.length === 0) {
        listEl.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem; color: var(--text-muted);">No hay solicitudes pendientes.</td></tr>';
        return;
    }

    listEl.innerHTML = data.map(s => {
        const statusClass = s.estado === 'aprobado' ? 'status-active' : s.estado === 'rechazado' ? 'status-danger' : 'status-pending';
        return `
            <tr>
                <td style="padding: 0.75rem 1rem;">
                    <strong>${s.perfiles?.alias || 'S/A'}</strong><br>
                    <small style="color: var(--text-muted);">${s.perfiles?.nombre_completo || 'S/N'}</small>
                </td>
                <td style="padding: 0.75rem 1rem;">${s.municipalidades?.nombre}</td>
                <td style="padding: 0.75rem 1rem;">
                    <a href="${s.documento_url}" target="_blank" class="button button--secondary btn-sm" style="display: inline-flex; align-items: center; gap: 4px;">
                        <i data-lucide="eye" style="width: 14px; height: 14px;"></i> Ver Doc
                    </a>
                </td>
                <td style="padding: 0.75rem 1rem;">
                    <span class="admin-status-badge ${statusClass}">${s.estado.toUpperCase()}</span>
                </td>
                <td style="padding: 0.75rem 1rem;">
                    <div style="display: flex; gap: 0.5rem;">
                        ${s.estado === 'pendiente' || s.estado === 'en_revision' ? `
                            <button class="button button--primary btn-sm btn-approve-role" data-id="${s.id}" title="Aprobar">
                                <i data-lucide="check" style="width: 14px; height: 14px;"></i>
                            </button>
                            <button class="button button--danger btn-sm btn-reject-role" data-id="${s.id}" title="Rechazar">
    ...
    document.querySelectorAll('.btn-approve-role').forEach(btn => {
        btn.onclick = () => gestionarSolicitudRol(btn.dataset.id, 'aprobado');
    });

    document.querySelectorAll('.btn-reject-role').forEach(btn => {
        btn.onclick = () => {
            const motivo = prompt('Ingresa el motivo del rechazo:');
            if (motivo !== null) gestionarSolicitudRol(btn.dataset.id, 'rechazado', motivo);
        };
    });
...
async function gestionarSolicitudRol(id, estado, comentarios = '') {
    const confirmacion = estado === 'aprobado' ? '¿Estás seguro de aprobar esta solicitud de rol municipal?' : '¿Estás seguro de rechazar esta solicitud?';
    if (!confirm(confirmacion)) return;
    ...
    try {
        const { error } = await supabaseClient
            ...
            .update({
                ...
                revisado_por: AuthModule.getUsuarioActual().id
            })
            .eq('id', id);

        if (error) throw error;

        mostrarMensaje(estado === 'aprobado' ? 'Solicitud aprobada: El usuario ahora es Municipal.' : 'Solicitud rechazada.', 'success');
        cargarSolicitudesRol();
        cargarDashboard(); // Actualizar contadores
    } catch (err) {
        Logger.error('Error al gestionar solicitud de rol:', err);
